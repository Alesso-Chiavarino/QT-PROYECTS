...
// Función inline para mostrar la marca de un instrumento
inline void mostrarMarca(const Instrumento* inst) {
    inst->verMarca();
}

// Función friend para imprimir los detalles de la Guitarra
class DetallesGuitarra {
public:
    friend void imprimirDetalles(const Guitarra& guitarra);
};

void imprimirDetalles(const Guitarra& guitarra) {
    std::cout << "Guitarra tiene " << guitarra.getCuerdas() << " cuerdas." << std::endl;
}
...
int main() {
    ...
    for (const auto& inst : instrumentos) {
        mostrarMarca(inst.get());
    }

    Guitarra guitarra;
    imprimirDetalles(guitarra);
